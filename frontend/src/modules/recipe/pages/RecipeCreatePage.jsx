...
import { useAllInventoryItems, useCreateRecipe } from '../hooks/useRecipes';
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 60px)', overflow: 'hidden', background: 'var(--color-gray-50)' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 16px 10px', borderBottom: '1px solid var(--color-gray-100)' }}>
          <div style={{ fontWeight: 700, fontSize: '0.8rem', color: 'var(--color-gray-500)', marginBottom: 10, letterSpacing: '0.07em', textTransform: 'uppercase' }}>
            🧂 Ingredients
          </div>
          <input
            type="text"
        ...
        <p style={{ padding: '8px 16px 4px', fontSize: '0.72rem', color: 'var(--color-gray-400)' }}>
          Drag item → drop into recipe area
        </p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 24px', background: '#fff', borderBottom: '1px solid var(--color-gray-200)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => navigate('/recipes')}>← Back</button>
          <div>
            <span style={{ fontWeight: 700, fontSize: '1.05rem', color: 'var(--color-gray-800)' }}>New Recipe</span>
            <span style={{ marginLeft: 12, fontSize: '0.8rem', color: 'var(--color-gray-400)' }}>Fill in the details and drag ingredients from the left panel</span>
          </div>
        </div>

        {/* Recipe fields row */}
        <div style={{ padding: '16px 24px 14px', background: '#fff', borderBottom: '1px solid var(--color-gray-100)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 0.7fr 0.7fr 0.7fr', gap: 14 }}>
            <div>
              <label className="form-label required" style={{ fontSize: '0.78rem' }}>Recipe Name</label>
              <input
                type="text"
                className="form-control"
            ...
            <div>
              <label className="form-label" style={{ fontSize: '0.78rem' }}>Meal Type</label>
              <select
                className="form-control"
            ...
            <div>
              <label className="form-label" style={{ fontSize: '0.78rem' }}>Food Type</label>
              <select
                className="form-control"
                value={fields.foodType}
                onChange={(e) => setFields((f) => ({ ...f, foodType: e.target.value }))}
                style={{ color: FOOD_TYPE_COLORS[fields.foodType] || 'inherit', fontWeight: 600 }}
              >
            ...
            <div>
              <label className="form-label required" style={{ fontSize: '0.78rem' }}>Yield Qty</label>
              <input
                type="number"
                className="form-control"
                min="0"
            ...
            <div>
              <label className="form-label required" style={{ fontSize: '0.78rem' }}>Yield Unit</label>
              <input
                type="text"
                className="form-control"
                placeholder="kg"
          ...
          <div style={{ marginTop: 10, maxWidth: 280 }}>
            <label className="form-label" style={{ fontSize: '0.78rem' }}>Cuisine Type <span style={{ fontWeight: 400, color: 'var(--color-gray-400)' }}>(optional)</span></label>
            <input
              type="text"
              className="form-control"
              placeholder="e.g. North Indian, South Indian..."
              value={fields.cuisineType}
              onChange={(e) => setFields((f) => ({ ...f, cuisineType: e.target.value }))}
            />
          </div>
        </div>
        ...
        >
          {ingredients.length === 0 ? (
            <div style={{
              ...
              background: isDragOver ? 'rgba(29,78,216,0.04)' : 'transparent',
              transition: 'all 0.2s',
              cursor: 'default',
            }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', borderRadius: 8, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                <thead>
                  <tr style={{ background: 'var(--color-gray-50)', borderBottom: '2px solid var(--color-gray-200)' }}>
                    {['#', 'Ingredient', 'Qty', 'Unit', 'Cost / Unit', 'Total Cost', ''].map((h, i) => (
                      <th key={i} style={{
                        padding: '11px 16px',
                      ...
                      <tr key={ing.inventoryItemId} style={{ borderBottom: '1px solid var(--color-gray-100)' }}>
                        ...
                        <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                          <button
                            ...
                            style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem', lineHeight: 1, padding: '2px 6px', borderRadius: 4 }}
                            title="Remove"
                          >×</button>
                        </td>
                      </tr>
        ...
        <div style={{
          ...
          <div style={{ color: 'var(--color-gray-500)', fontSize: '0.875rem' }}>
            ...
            {ingredients.length > 0 && (
              <> &nbsp;·&nbsp; Total: <strong style={{ color: 'var(--color-primary)' }}>{fmt(totalCost)}</strong></>
            )}
          </div>
